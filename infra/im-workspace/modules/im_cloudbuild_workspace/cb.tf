...
locals {
  ...
  default_preview_steps = [
    ...
    { id = "terraform_init", name = local.tf_cloudbuilder_image, args = ["init", "-no-color"], dir = var.im_deployment_repo_dir != "" ? var.im_deployment_repo_dir : null },
    { id = "terraform_show", name = local.tf_cloudbuilder_image, args = ["show", "/workspace/plan.tfplan", "-no-color"], dir = var.im_deployment_repo_dir != "" ? var.im_deployment_repo_dir : null },
  ]
